...
int load_config_file(wchar_t * path)
{
    ...
    if (_wfopen_s(&file, path, L"r") != 0) {  // Alterado para != 0
...
void put_config_path(wchar_t * path)
{
    ...
    PathRemoveFileSpecW(path); // Isso remove 'dual-key-remap.exe' do caminho

    // Agora o caminho é o diretório onde o executável está localizado
    wcscat(path, L"\\config.txt"); // Adiciona o nome do arquivo de configuração
}
...
int main()
{
    ...
    if (!PathFileExistsW(config_path)) {
        printf("Configuration file '%ws' does not exist.\n", config_path);
        goto end;
    }
